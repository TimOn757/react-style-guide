...
```
Вместо пути в таком формате - state.usersPage.portionSize, - можно использовать прописанные заранее селекторы, которые модно хранить в папке selectors, в которой находятся файлы с селекторами к определенным redusers (например: userSelectors.ts, profileSelectors.ts и т.п.). Папка slectors хранится в папке redux. 
...
Используется вместо connect, при помощи которого мы прокидываем необходимые функции контейнернцю компоненту, а из нее при помощи props в презентационную.
useDispatch используется прямо в той компоненте, где необходим функционал. Таким образом код сокращается.
